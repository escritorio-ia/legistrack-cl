async function mapAllSenadoComisiones() {
  ...
  console.log(`Total comisiones del Senado en comisiones.php: ${comisiones.length}`);
  const permanentes = comisiones.filter(c => c.tipo.toLowerCase().includes("permanente"));
  console.log(`\n=== COMISIONES PERMANENTES (${permanentes.length}) ===`);
  for (const c of permanentes) {
    console.log(`ID ${c.id}: ${c.nombre} (email: ${c.email})`);
    c.integrantes.forEach(i => {
      console.log(`  - ${i.rol}: ${i.nombre} (${i.cargo}) [${i.email}]`);
    });
  }
}

mapAllSenadoComisiones().catch(console.error);
